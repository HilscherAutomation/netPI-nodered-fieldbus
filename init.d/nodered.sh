...
then
	...
	#copy netHAT default firmware to location where driver will load it from
	if [ ! -f /opt/cifx/deviceconfig/FW/channel0/*.nxf ]; then
		cp /root/.node-red/FWPool/X070D000.nxf /opt/cifx/deviceconfig/FW/channel0/X070D000.nxf
	fi	
...
then
	...
	#copy netPI default firmware to location where driver will load it from
	if [ ! -f /opt/cifx/deviceconfig/FW/channel0/*.nxf ]; then
		cp /root/.node-red/FWPool/R160D000.nxf /opt/cifx/deviceconfig/FW/channel0/R160D000.nxf
	fi	
else
